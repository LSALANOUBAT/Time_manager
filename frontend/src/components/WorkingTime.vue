...
export default {
  ...
  methods: {
    ...
    async updateWorkingTime() {
      ...
      const response = await fetch(`https://web.orbesle.fr:4000/api/workingtime/${this.workingTimeId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          workingtime: {
    ...
    async deleteWorkingTime() {
      ...
      const response = await fetch(`https://web.orbesle.fr:4000/api/workingtime/${this.userId}/${this.workingTimeId}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' }
      });
